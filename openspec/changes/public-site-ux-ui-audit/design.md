...
### D7 — Language switcher: keep the `<select>`, add a `<noscript>` link list, move CSS to a stylesheet
Keep the existing `<select onchange>` (already covered by `ExampleTest`), add short display codes to `config/locales.php` (`'short_labels' => ['en' => 'EN', 'uk' => 'UA', 'pl' => 'PL']`) so the `uk → UA` mapping is configuration, not a ternary in Blade, and render a `<noscript>` list of localized links as the scripting-free path. Move the injected `<style>` block into a new `public/css/site.css` linked after `style.css` in `layouts/main.blade.php`; keep the mobile placement rules but verify against 320/360/390 px widths during apply.
