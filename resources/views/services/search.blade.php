...
@section('content')
    ...
    <section class="breadcrumbs-custom">
        <div class="breadcrumbs-custom__aside bg-image context-dark"
            ...
            <div class="container">
                <h1 class="breadcrumbs-custom__title">{{ __('site.services_search') }}</h1>
            </div>
        </div>
                    ...
                    @foreach($services as $item)
                        ...
                        <article class="post-single">
                            <h4 class="post-single__title">
                                <a href="{{ route('category-service', [$item->serviceCategory->slug, $item->slug]) }}">{{ $item->title }}</a>
                            </h4>
                            ...
                            <a class="button button-sm button-default button-ujarak"
                               href="{{ route('category-service', [$item->serviceCategory->slug, $item->slug]) }}">{{ __('site.read_more') }}</a>
                        </article>
                    @endforeach
